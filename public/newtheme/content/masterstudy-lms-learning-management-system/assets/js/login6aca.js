      ...
      methods: {
        ...
        lostPassword: function lostPassword() {
          ...
          this.$http.post(stm_lms_ajaxurl + '?action=stm_lms_lost_password&nonce=' + stm_lms_nonces['stm_lms_lost_password'], data).then(function (response) {
            // Hard Coded Solution Not Good Aproach But This sentese was not found
            if(response.body['message'] == "Please fill email field"){
              response.body['message'] = "Rellena el campo con tu email ";
            }
            vm.message = response.body['message'];
            vm.status = response.body['status'];
