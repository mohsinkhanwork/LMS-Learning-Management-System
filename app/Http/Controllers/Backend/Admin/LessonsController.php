...
class LessonsController extends Controller
{
    ...
    public function create()
    {
        ...
        $topics = Topic::get()->where('courses_id',request()->course_id)->where('published', 1)->pluck('title','id')->prepend('Please select','');

        
        return view('backend.lessons.create', compact('courses' , 'topics'));
    }
    ...
    public function store(StoreLessonsRequest $request)
    {
        ...
        //start audio google audio text to auido
        
        if($request->full_text != NULL){
        $text = strip_tags($request->full_text);
        ...
        if (strlen($text) < $max) {
            ...
            $txt = chunk_split($text, $max);
            $txt = explode("||",wordwrap($text, 4990, "||"));
        }
            }
        putenv('GOOGLE_APPLICATION_CREDENTIALS='.public_path('textTospeech/text.json'));
        for ($i=0; $i < count($txt); $i++) {
            $textToSpeechClient = new TextToSpeechClient();
            $input = new SynthesisInput();
            $input->setText(str_replace('\r\n', '',strip_tags($txt[$i])));
            $voice = new VoiceSelectionParams();
            $voice->setLanguageCode('es-ES');
            $voice->setName('es-ES-Standard-A');
        ...
        //make one audio
        $audio1 = \falahati\PHPMP3\MpegAudio::fromFile(public_path($lesson->id.'test0.mp3'))->stripTags();
        if(file_exists(public_path($lesson->id.'test1.mp3'))){
        $audio2 = \falahati\PHPMP3\MpegAudio::fromFile(public_path($lesson->id.'test1.mp3'))->stripTags();
        }
        if(file_exists(public_path($lesson->id.'test2.mp3'))){
        $audio3 = \falahati\PHPMP3\MpegAudio::fromFile(public_path($lesson->id.'test2.mp3'))->stripTags();
        }
        if(file_exists(public_path($lesson->id.'test3.mp3')) ){
        $audio4 = \falahati\PHPMP3\MpegAudio::fromFile(public_path($lesson->id.'test3.mp3'))->stripTags();
        }
        if(file_exists(public_path($lesson->id.'test4.mp3')) ){
        $audio5 = \falahati\PHPMP3\MpegAudio::fromFile(public_path($lesson->id.'test4.mp3'))->stripTags();
        }
        if(file_exists(public_path($lesson->id.'test5.mp3')) ){
        $audio6 = \falahati\PHPMP3\MpegAudio::fromFile(public_path($lesson->id.'test5.mp3'))->stripTags();
        }
        if(file_exists(public_path($lesson->id.'test6.mp3')) ){
        $audio7 = \falahati\PHPMP3\MpegAudio::fromFile(public_path($lesson->id.'test6.mp3'))->stripTags();
        }
         $first =  $audio1->saveFile("test".$lesson->id.".mp3");
         if(file_exists(public_path($lesson->id.'test1.mp3')) ){
             $second = $first->append($audio2)->saveFile("test".$lesson->id.".mp3");
        }
         if(file_exists(public_path($lesson->id.'test2.mp3')) ){
            $third = $second->append($audio3)->saveFile("test".$lesson->id.".mp3");
        }
         if(file_exists(public_path($lesson->id.'test3.mp3')) ){
            $fourth = $third->append($audio4)->saveFile("test".$lesson->id.".mp3");
        }
        if(file_exists(public_path($lesson->id.'test4.mp3')) ){
            $five = $fourth->append($audio5)->saveFile("test".$lesson->id.".mp3");
        }
        if(file_exists(public_path($lesson->id.'test5.mp3')) ){
            $six = $five->append($audio6)->saveFile("test".$lesson->id.".mp3");
        }
        if(file_exists(public_path($lesson->id.'test6.mp3')) ){
            $seven = $six->append($audio7)->saveFile("test".$lesson->id.".mp3");
        }
        //end make one audio
        }
        ...
        if ($lesson->published == 1) {
            ...
            $timeline->topic_id = $request->topic_id;
    ...
    public function edit($id)
    {
        ...
        $topics = Topic::get()->where('courses_id',$lesson->course->id)->where('published', 1)->pluck('title','id')->prepend('Please select','');

        return view('backend.lessons.edit', compact('lesson', 'courses', 'videos', 'topics'));
    }
