...
class TestsController extends Controller
{
        ...
        return DataTables::of($tests)
            ...
            ->addColumn('questions', function ($q) {
                if (count($q->questions) > 0) {
                    return "<span>".count($q->questions)."</span><a class='btn btn-success float-right' href='".route('admin.questions.index', ['test_id'=>$q->id])."'><i class='fas fa-arrow-right'></i></a> ";
                }
    ...
    public function create()
    {
        ...
        $topics = Topic::get()->where('courses_id',request()->course_id)->pluck('title','id')->prepend('Please select','');

        // dd(request()->course_id);

        return view('backend.tests.create', compact('courses', 'lessons', 'topics' ));
    }
    ...
    public function edit($id)
    {
        ...
        $topics = Topic::get()->where('courses_id',$test->course->id)->pluck('title','id')->prepend('Please select','');


        return view('backend.tests.edit', compact('test', 'courses', 'lessons', 'topics'));
    }
    ...
    public function update(UpdateTestsRequest $request, $id)
    {
        ...
        return redirect()->route('admin.tests.index',['course_id' => $request->course_id])->withFlashSuccess(trans('alerts.backend.general.updated'));
    }
