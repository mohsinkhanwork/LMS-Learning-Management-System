...
public function student_index(Request $request)
    {
        ...
        $teachers = User::role('administrator')->get()
            ->where('id', '!=', auth()->user()->id)
            ->pluck('name', 'id');
        ...
        if ($agent->isMobile()) {
            $view = 'backend.messages.student-index-mobile';
        } else {
            $view = 'backend.messages.student-index-desktop';
        }
        return view($view, [
            'threads' => $threads,
            'teachers' => $teachers,
 ...
 public function student_send(Request $request)
    {
        ...
        return redirect(route('admin.student_messages').'?thread='.$thread->id);
    }
 ...
 public function student_reply(Request $request)
    {
        $this->validate($request, [
            'message' => 'required'
        ], [
        ...
        return redirect(route('admin.student_messages').'?thread='.$message->thread_id)->withFlashSuccess('Message sent successfully');
    }
...
public function student_getUnreadMessages(Request $request)
    {
        $unreadMessageCount = auth()->user()->unreadMessagesCount();
        $unreadThreads = [];
        foreach (auth()->user()->threads as $item) {
